...
// @version     2.0
// @author      Gureumi
// @description 2024. 3. 17. 오후 2:20:00
// @license MIT
...
var shopName = shopMatch ? shopMatch[0].match(/^(?:https?:\/\/)?([^\.]+)\.booth\.pm\//)[1] : document.evaluate("/html/body/div[5]/main/div[2]/article/div/div/div/div[4]/div[1]/div[1]/div[1]/header/a/@href", document, null, XPathResult.STRING_TYPE, null).stringValue.match(/^(?:https?:\/\/)?([^\.]+)\.booth\.pm\//)[1];
...
if (productId != null) {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(xhr) {
      ...
      if (shopMatch != null) {
        co = document.querySelector(".main-info-column > div:nth-child(3)");
      }
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(xhr) {
      ...
      result += "<span style='float:left; width: 100%; margin-bottom: 50px;'>리뷰 추가는 <a href='https://vrc-booth.com/shop/" + shopName + "'>여기</a>에서 할 수 있습니다.</span></div>";

      var co = document.querySelector(".main-info-column")
      if (shopMatch != null) {
        co = co = document.querySelector(".main-info-column > div:nth-child(3)");
      }
      co.insertAdjacentHTML("afterbegin", result);
      console.log(co);
